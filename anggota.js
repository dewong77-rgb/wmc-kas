...
// REKAP - 5 besar dengan filter bulan berdasarkan tanggal surat
function renderRekapAnggota() {
  ...
  var filterHTML = '<div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">'
  ...
  var filtered = allKegiatan.filter(function(k) {
    ...
    var tgl = k.tanggal_surat ? new Date(k.tanggal_surat) : null;
    if (!tgl) return false;
  ...
  var listHTML = top5.length ? top5.map(function(a, i) {
  ...
  : '<div style="color:var(--text3);font-size:13px;padding:16px 0;text-align:center">Belum ada data untuk periode ini</div>';
...
// KEGIATAN - diurutkan berdasarkan tanggal surat terbaru
