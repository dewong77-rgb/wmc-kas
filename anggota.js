...
function renderRekapAnggota() {
  ...
  // Stats per divisi:
  // anggotaSet: semua anggota terdaftar di divisi
  // pernahIkutSet: pernah ikut kegiatan (hadir atau pinjam)
  // hadirFisikSet: pernah hadir fisik minimal sekali
  // hanyaPinjamSet: ikut kegiatan tapi TIDAK PERNAH hadir fisik, selalu pinjam nama
  // hadir: total kehadiran fisik
  // pinjam: total pinjam nama
  var divisiStats = {};

  // Inisialisasi divisi dari allAnggota
  allAnggota.forEach(function(a) {
    var divisi = a.jabatan || 'Tanpa Divisi';
    if (!divisiStats[divisi]) divisiStats[divisi] = { anggotaSet: {}, pernahIkutSet: {}, hadirFisikSet: {}, hadir: 0, pinjam: 0 };
    divisiStats[divisi].anggotaSet[a.id] = true;
  });

  // Isi dari kegiatan yang sudah difilter
  ...
  var timHTML = divisiList.map(function(d) {
    ...
    // Pernah ikut tapi tidak pernah hadir fisik (selalu pinjam nama)
    ...
    return '<div style="padding:12px 0;border-bottom:1px solid var(--border)">'
      ...
      + (jmlBelumIkut > 0 ? '<span style="color:var(--red)">✗ ' + jmlBelumIkut + ' belum pernah ikut</span>' : '<span style="color:var(--green)">semua sudah ikut</span>')
      + '</div>'
      // Progress bar
      + (totalKegiatan > 0
    ...
    rankHTML = '<div style="font-size:12px;font-weight:700;color:var(--text2);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:10px">'
      ...
      + slice.map(function(a, i) {
          ...
          var pinjamText = a.pinjam > 0
            ? (hadirFisik === 0
              ? ' · <span style="color:var(--red)">selalu pinjam nama</span>'
              : ' · <span style="color:var(--yellow)">' + a.pinjam + 'x pinjam</span>')
            : '';
