...
function renderRekapAnggota() {
  ...
  // Hitung stats anggota + divisi
  var stats = {};
  var divisiStats = {}; // { divisi: { anggota: Set, hadir: 0, pinjam: 0, aktif: Set } }
  ...
  filtered.forEach(function(k) {
    ...
    k.kegiatan_peserta.forEach(function(p) {
      ...
      // Stats per divisi
      if (!divisiStats[divisi]) divisiStats[divisi] = { anggotaSet: {}, aktifSet: {}, hadir: 0, pinjam: 0 };
      divisiStats[divisi].anggotaSet[aid] = true;
      divisiStats[divisi].aktifSet[aid] = true;
      if (isPinjam) divisiStats[divisi].pinjam++;
      else divisiStats[divisi].hadir++;
    });
  });

  // Tambah anggota yang tidak ikut kegiatan apapun ke divisiStats
  ...
  // Card ringkasan
  var ringkasanHTML = '';
  if (filtered.length > 0 || allAnggota.length > 0) {

    // Breakdown per tim — urutkan dari yang paling banyak hadir
    var divisiList = Object.keys(divisiStats).sort(function(a, b) {
      return divisiStats[b].hadir - divisiStats[a].hadir;
    });

    var maxHadir = divisiList.reduce(function(m, d) { return Math.max(m, divisiStats[d].hadir); }, 1);
      ...
      return '<div style="padding:10px 0;border-bottom:1px solid var(--border)">'
        ...
        + (totalKegiatan > 0
          ? '<div style="display:flex;height:8px;border-radius:4px;overflow:hidden;margin-bottom:4px">'
            + '<div style="width:' + pctHadir + '%;background:var(--green);transition:width 0.3s"></div>'
            + '<div style="width:' + pctPinjam + '%;background:var(--yellow);transition:width 0.3s"></div>'
  ...
  allKegiatan.forEach(function(k) {
    ...
    k.kegiatan_peserta.forEach(function(p) {
      if (p.anggota && p.anggota.nama === nama) {
        histori.push({
          nama_kegiatan: k.nama, tipe: k.tipe,
          tanggal: k.tanggal_mulai, lokasi: k.lokasi,
