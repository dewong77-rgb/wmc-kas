...
// DASHBOARD
var dashBulan = -1; // -1 = semua
var dashTahun = -1;

function renderDashboard() {
  var now = new Date();

  // Kumpulkan tahun dari allTrx
  var years = {};
  ...
  // Init default tahun jika belum diset
...
function setDashFilter(bulan, tahun) {
  dashBulan = bulan; dashTahun = tahun;
  renderDashboard();
}
...
function renderTopKategori(trxFiltered) {
  function topKat(jenis, elId, titleEl, emptyMsg) {
    var map = {};
  ...
  topKat('masuk', 'top-masuk', 'top-masuk-title', 'Tidak ada pemasukan');
  topKat('keluar', 'top-keluar', 'top-keluar-title', 'Tidak ada pengeluaran');
}

function renderTopKegiatan(trxFiltered) {
  var el = document.getElementById('top-kegiatan');
  if (!el) return;
  // Hitung pemasukan per kegiatan
  var map = {};
  trxFiltered.filter(function(t){ return t.jenis === 'masuk' && t.kegiatan_id; }).forEach(function(t) {
    ...
    if (!map[keg]) map[keg] = { nama: keg, total: 0, count: 0 };
    map[keg].total += t.nominal;
    map[keg].count++;
  });
  var sorted = Object.values(map).sort(function(a,b){ return b.total - a.total; });
  if (!sorted.length) {
    el.innerHTML = '<div style="color:var(--text3);font-size:13px;padding:8px 0">Tidak ada pemasukan terkait kegiatan</div>';
    return;
  }
  var maxVal = sorted[0].total;
...
function openEditTrx(id) {
  ...
  if (t.jenis === 'masuk') { populateKegiatanSelect(t.kegiatan_id || ''); }
  var kegWrap = document.getElementById('trx-kegiatan-wrap');
  if (kegWrap) kegWrap.style.display = t.jenis === 'masuk' ? 'block' : 'none';
  ...
  // Buka sheet langsung tanpa reset form
...
function resetTrxForm() {
  if (editingTrxId) return; // jangan reset kalau sedang edit
...
function setJenis(jenis) {
  ...
  renderKatGrid();
  // Tampilkan dropdown kegiatan hanya untuk uang masuk
  var kegWrap = document.getElementById('trx-kegiatan-wrap');
  if (kegWrap) {
...
function populateKegiatanSelect(selectedId) {
  ...
  if (!sel) return;
  // Urutkan kegiatan dari terbaru berdasarkan tanggal surat
  ...
  // Auto-isi keterangan saat kegiatan dipilih
...
async function submitTrx() {
  ...
  var payload = { jenis: currentJenis, nominal: nominal, tanggal: tanggal, posisi_kas_id: kasId, kategori_id: selectedKatId, keterangan: ket, status: status, kegiatan_id: currentJenis === 'masuk' ? kegiatanId : null };
  var r, trxId;
  if (editId) {
