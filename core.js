...
function goPage(name) {
  ...
  var titles = {
    dashboard:'WMC Kas', transaksi:'Transaksi', anggaran:'Pos Anggaran',
  ...
  if (name === 'anggaran') renderPosAnggaranPage();
  if (name === 'anggota') {
...
async function loadAll() {
  await Promise.all([
    loadKategori(), loadKas(), loadTrx(),
    loadAnggota(), loadAnggaran(),
    loadPosAnggaran(), loadAlokasiMasuk()
  ]);
  renderDashboard();
  renderTrxList();
  populateKasSelect();
...
async function loadTrx() {
  var r = await db.from('transaksi')
    .select('*, kategori(nama,warna,jenis), posisi_kas(nama,tipe), kegiatan(nama), pos_anggaran(nama,warna)')
    .neq('status', 'batal')
...
async function loadPosAnggaran() {
  var r = await db.from('pos_anggaran').select('*').eq('aktif', true).order('urutan');
  allPosAnggaran = r.data || [];
}
async function loadAlokasiMasuk() {
  var r = await db.from('alokasi_masuk')
    .select('*, pos_anggaran(nama,warna), transaksi(nominal,tanggal,keterangan)')
    .order('created_at', { ascending: false });
  allAlokasiMasuk = r.data || [];
}
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  ['trx','detail','anggaran','anggota','kegiatan','user','kat','kas','pos','alokasi'].forEach(function(t) {
