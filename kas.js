...
function trxHTML(t) {
  ...
  var hasBukti = t.bukti_url ? ' \u00b7 \ud83d\udcce' : '';
  return '<div class="trx-item" onclick="openDetail(\'' + t.id + '\')">'
    + '<div class="trx-dot ' + t.jenis + '"></div>'
    + '<div class="trx-info"><div class="trx-ket">' + (t.keterangan || katNama) + '</div><div class="trx-meta">' + tgl + ' \u00b7 ' + katNama + kegNama + posNama + hasBukti + '</div></div>'
...
function openDetail(id) {
  ...
  document.getElementById('detail-content').innerHTML =
    ...
    + (t.bukti_url ? '<div class="detail-row"><div class="detail-key">Bukti</div><div class="detail-val"><a href="' + t.bukti_url + '" target="_blank" style="color:var(--accent);font-weight:600">\ud83d\udcce Buka Bukti</a></div></div>' : '');
  ...
  if (btnAlokasi) {
    var sudahAlokasi = allAlokasiMasuk && allAlokasiMasuk.some(function(a){ return a.transaksi_id === t.id; });
    btnAlokasi.style.display = (t.jenis === 'masuk' && canEdit && allPosAnggaran && allPosAnggaran.length > 0) ? 'block' : 'none';
...
function openEditTrx(id) {
  ...
  var buktiLinkEl = document.getElementById('trx-bukti-link');
  if (buktiLinkEl) buktiLinkEl.value = t.bukti_url || '';
  ...
  if (t.jenis === 'masuk') {
    ...
    if (posWrap2) { posWrap2.style.display = 'block'; populatePosSelect(t.pos_id || ''); }
  }
...
function resetTrxForm() {
  ...
  var buktiLinkEl = document.getElementById('trx-bukti-link');
  if (buktiLinkEl) buktiLinkEl.value = '';
...
async function submitTrx() {
  ...
  var buktiLink = document.getElementById('trx-bukti-link') ? document.getElementById('trx-bukti-link').value.trim() || null : null;
  ...
  if (currentJenis === 'masuk' && !editId && allPosAnggaran && allPosAnggaran.length > 0 && trxId) {
    setTimeout(function() { openSheetAlokasi(trxId, nominal); }, 500);
  }
