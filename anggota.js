...
function renderRekapAnggota() {
  ...
  filtered.forEach(function(k) {
    ...
    k.kegiatan_peserta.forEach(function(p) {
      ...
      // Ambil jabatan/divisi dari allAnggota
      var anggotaData = allAnggota.filter(function(a){ return a.id === aid; })[0];
      var divisi = anggotaData ? (anggotaData.jabatan || '') : '';
  ...
  Object.values(stats).forEach(function(a) {
    var div = a.divisi || 'Lainnya';
    if (!divisiMap[div]) divisiMap[div] = 0;
    divisiMap[div]++;
  });
  var divisiList = Object.keys(divisiMap).sort();

  var totalPesertaUnik = Object.keys(stats).length;
  ...
  if (filtered.length > 0) {
    ringkasanHTML = '<div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:14px;margin-bottom:16px">'
      ...
      + '</div>'
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px">'
      + '<div style="background:var(--bg2);border-radius:8px;padding:8px;text-align:center"><div style="font-size:20px;font-weight:800;color:var(--text)">' + totalPesertaUnik + '</div><div style="font-size:10px;color:var(--text3);margin-top:2px">Total Anggota</div></div>'
      ...
      // Breakdown divisi
      + (divisiList.length > 1
        ? '<div style="font-size:11px;color:var(--text3);margin-bottom:6px">Per tim/divisi:</div>'
      ...
      // Bar chart per bulan
      + (bulanList.length > 1
        ? '<div style="font-size:11px;color:var(--text3);margin-bottom:6px">Per bulan:</div>'
...
function openDetailAnggota(nama) {
  // Kumpulkan semua kegiatan yang diikuti anggota ini
  var histori = [];
  allKegiatan.forEach(function(k) {
    if (!k.kegiatan_peserta) return;
    k.kegiatan_peserta.forEach(function(p) {
      if (p.anggota && p.anggota.nama === nama) {
        histori.push({
          ...
          tanggal_surat: k.tanggal_surat,
          lokasi: k.lokasi,
          status: p.status_kehadiran || 'hadir'
        });
      }
    });
  });
  ...
  var content = '<div style="display:flex;gap:12px;margin-bottom:16px">'
    ...
    + '<div style="flex:1;background:var(--bg2);border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:800;color:var(--green)">' + totalHadir + '</div><div style="font-size:11px;color:var(--text3)">Hadir</div></div>'
    + '<div style="flex:1;background:var(--bg2);border-radius:8px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:800;color:var(--yellow)">' + totalPinjam + '</div><div style="font-size:11px;color:var(--text3)">Pinjam</div></div>'
    + '</div>';

  if (!histori.length) {
    content += '<div style="color:var(--text3);text-align:center;padding:24px">Belum ada kegiatan</div>';
  } else {
    content += histori.map(function(h) {
...
function renderDataAnggota() {
  ...
  var searchBar = '<div style="position:relative;margin-bottom:12px">'
    ...
    + '<input type="search" placeholder="Cari nama anggota..." value="' + escQ(searchAnggota) + '" oninput="onSearchAnggota(this.value)" style="width:100%;background:var(--surface);border:1.5px solid var(--border);border-radius:var(--radius-sm);padding:10px 14px 10px 38px;color:var(--text);font-family:var(--font);font-size:14px;outline:none">'
    + '</div>';
  ...
  el.innerHTML = divisiHTML + searchBar + slice.map(function(a) {
    return '<div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-sm);padding:14px 16px;display:flex;align-items:center;gap:12px;margin-bottom:8px">'
      ...
      + '<div style="font-size:12px;color:var(--text2);margin-top:2px">' + (a.jabatan || '-') + (a.kontak ? ' · ' + a.kontak : '') + '</div>'
      + '</div>'
...
function renderKegiatanList() {
  ...
  var filtered = sorted.filter(function(k) {
    ...
    // Filter bulan/tahun berdasarkan tanggal surat
  ...
  if (!filtered.length) {
    el.innerHTML = searchBar + '<div style="color:var(--text3);font-size:13px;padding:16px 0;text-align:center">Tidak ada kegiatan ditemukan</div>';
    return;
  }
